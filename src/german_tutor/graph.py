...
SPECIALIST_PROMPTS: dict[str, str] = {
    ...
    "lesson": (
        ...
        "it and call cache_lesson so future resumes are identical. The cached lesson "
        "JSON must include a top-level lesson_objective string derived from the unit "
        "can_do goal. For every teach/checkpoint step, include at least one complete "
        "German example sentence paired with an English gloss in the cached "
        "examples list; do not leave any example without its gloss. For every "
        "teach/checkpoint step, include glossed example "
        "sentences (German + English gloss), a short beginner_safe_explanation in "
        "plain English, a pattern_scope line that says this is the small pattern for "
        "this lesson rather than a global rule, and a short check_understanding "
        "prompt that asks the learner to confirm the key distinction before "
        "advancing. Include at least one practice item in the cached lesson with an "
        "expected_answer plus likely_mistakes or correction_guidance so the tutor can "
        "give specific corrective feedback when the learner answers partially or "
        "incorrectly. Prefer simple A1 metalanguage: say 'for names' or 'for this "
        "pattern' instead of abstract terms like 'identity', and explain wie here as "
        "the question word in 'Wie heißt du?' rather than with broader dictionary-"
        "style glosses unless clearly flagged as extra information. Each step should "
        "surface exactly one learner-facing action: either a brief understanding "
        "check or a production practice item, but never both in the same turn. "
        "Checkpoint steps may use either action type, but must store only one of "
        "check_understanding or practice for that step. After each step "
        ...
        "replace the example with one that only uses already-explained German. When "
        "teaching from a cached lesson, surface the stored lesson_objective near the "
        "start of the lesson and preserve the stored glossed examples, "
        "beginner_safe_explanation, pattern_scope, micro_vocabulary, "
        "success_feedback, advancement_note, contrast_target, success_criteria, "
        "required_glossed_example, partial_credit_guidance, practice metadata, and "
        "check_understanding moments instead of paraphrasing them away. Every lesson "
        "turn should include: (1) the current step focus, (2) at least one German "
        "example with an English gloss on the same turn, preferably the stored "
        "required_glossed_example when one is present, (3) a small visible "
        "micro_vocabulary list from the stored step fields, and (4) an explicit brief "
        "understanding check before you move to a new point, unless this step is the "
        "single production practice step for that turn. If the learner gives a "
        "wrong or partial answer, use the stored correction guidance and "
        "partial_credit_guidance to name the specific fix, show the corrected German, "
        "and briefly explain what to notice. If a contrast_target is stored, do not "
        "treat a one-sided answer as fully correct: explicitly say which side was "
        "right and which distinction is still missing. If the learner answers "
        "correctly, use the stored success_feedback cue plus the stored "
        "success_criteria to give brief response-specific praise tied to the exact "
        "German they used and why it met the step goal. "
        "When a step is complete, use the stored advancement_note to explicitly close "
        "what the learner demonstrated before moving on. Keep the step small but not "
        "skeletal. " + LEVEL_RULE + TTS_HINT + MOMENTUM + GROUND_HINT
    ),
    "grammar": (
